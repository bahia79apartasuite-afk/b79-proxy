# Plano 14 — busto retrovisor

**Video** `on_the_road` · **In** 23.63 s · **Out** 24.17 s · **Duracion** 0.53 s (16 frames)  
**Tipo** busto · **Camara** fija sobre el retrovisor interior · **Sujeto** PROTAGONISTA reflejado  
**Frame de referencia del original:** `analysis/on_the_road/frames/t_23.85.png`

> **Por que existe este plano:** Truco de continuidad: el rostro vuelve enmarcado, sin gastar un plano de cara entero.
...
Close-up of the interior rear-view mirror, THE DRIVER's face reflected in it, the orange desert and the road receding behind him through the rear window. The mirror housing and the top of the windscreen frame the shot. Warm light from the front.
```

**Que lleva y en que orden:** bloque de estilo → bloque de EL CONDUCTOR → bloque de CARRETERA MESETAS → encuadre y accion congelada. El orden importa: los modelos pesan mas el principio del prompt, y el estilo es lo que no puede fallar.

**Si usas Elements de Higgsfield:** en vez de pegar los bloques enteros, escribe `<<<id_de_el_conductor>>>`, `<<<id_de_carretera_mesetas>>>` dentro del prompt y deja solo el bloque de estilo y el encuadre. Los ids estan en `characters/el_conductor/element.txt`.
...
REFERENCES: start frame attached as start_image. Character/location sheets attached as image_references: EL CONDUCTOR, CARRETERA MESETAS.
ACTION: His eyes flick up into the mirror, hold for a beat, and go back to the road. The landscape behind slides past continuously.
CAMERA: static shot, locked off, mounted in the car.
...
SFX: engine under load, wind noise, tyre roar.
```
...
|---|---|---|
| `duration` | 4 | El plano dura 0.53 s, pero Seedance no genera clips de menos de 4 s. Se generan 4 s y en el montaje se recortan 0.53 s del centro, que es donde el movimiento ya esta asentado. |
...
> **Duracion:** **El plano dura 0.53 s, pero Seedance no genera clips de menos de 4 s.** Se generan 4 s y en el montaje se recortan 0.53 s del centro, que es donde el movimiento ya esta asentado. Los primeros frames de un clip generado casi siempre arrancan con un titubeo.
...
- [ ] ¿Hay al menos 0.53 s utiles seguidos, sin el titubeo del arranque?
...
**Anterior:** plano 13 · **Siguiente:** plano 15  
