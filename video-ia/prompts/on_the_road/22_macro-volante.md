# Plano 22 — macro volante

**Video** `on_the_road` · **In** 31.47 s · **Out** 32.13 s · **Duracion** 0.67 s (20 frames)  
**Tipo** macro · **Camara** macro fija · **Sujeto** PROP volante y manos  
**Frame de referencia del original:** `analysis/on_the_road/frames/t_31.73.png`

> **Por que existe este plano:** Devuelve el cuerpo al plano sin volver a la cara.
...
Close-up of two dark-skinned hands at ten and two on a three-spoke wheel with a polished drilled aluminium centre and a worn dark wood rim, a black leather watch on the left wrist, the gauge cluster far out of focus beyond. Warm light from the windscreen. The face is NOT in frame.
```

**Que lleva y en que orden:** bloque de estilo → bloque de INSERTOS COCHE → bloque de EL CONDUCTOR → encuadre y accion congelada. El orden importa: los modelos pesan mas el principio del prompt, y el estilo es lo que no puede fallar.

**Si usas Elements de Higgsfield:** en vez de pegar los bloques enteros, escribe `<<<id_de_insertos_coche>>>`, `<<<id_de_el_conductor>>>` dentro del prompt y deja solo el bloque de estilo y el encuadre. Los ids estan en `props/insertos_coche/element.txt`.
...
REFERENCES: start frame attached as start_image. Character/location sheets attached as image_references: INSERTOS COCHE, EL CONDUCTOR.
ACTION: The hands make one small correction to the right and settle back. The wheel vibrates continuously under them.
...
SFX: road vibration through the column, engine.
```
...
|---|---|---|
| `duration` | 4 | El plano dura 0.67 s, pero Seedance no genera clips de menos de 4 s. Se generan 4 s y en el montaje se recortan 0.67 s del centro, que es donde el movimiento ya esta asentado. |
...
> **Duracion:** **El plano dura 0.67 s, pero Seedance no genera clips de menos de 4 s.** Se generan 4 s y en el montaje se recortan 0.67 s del centro, que es donde el movimiento ya esta asentado. Los primeros frames de un clip generado casi siempre arrancan con un titubeo.
...
- [ ] ¿Hay al menos 0.67 s utiles seguidos, sin el titubeo del arranque?
...
**Anterior:** plano 21 · **Siguiente:** plano 23  
