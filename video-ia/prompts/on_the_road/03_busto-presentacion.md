# Plano 03 — busto presentacion

**Video** `on_the_road` · **In** 3.07 s · **Out** 4.70 s · **Duracion** 1.63 s (49 frames)  
**Tipo** busto · **Camara** fija, push in lento · **Sujeto** PROTAGONISTA  
**Frame de referencia del original:** `analysis/on_the_road/frames/t_03.72.png`

> **Por que existe este plano:** Revelacion del personaje. Llega tarde a proposito: ya te importaba.
...
Bust portrait of THE DRIVER seated at the diner counter, front view, chewing slowly, eyes looking off camera-right, mouth closed between bites. The diner interior and a cook far out of focus behind. Warm window light from camera-left.
```

**Que lleva y en que orden:** bloque de estilo → bloque de EL CONDUCTOR → bloque de DINER DESIERTO → encuadre y accion congelada. El orden importa: los modelos pesan mas el principio del prompt, y el estilo es lo que no puede fallar.

**Si usas Elements de Higgsfield:** en vez de pegar los bloques enteros, escribe `<<<id_de_el_conductor>>>`, `<<<id_de_diner_desierto>>>` dentro del prompt y deja solo el bloque de estilo y el encuadre. Los ids estan en `characters/el_conductor/element.txt`.
...
REFERENCES: start frame attached as start_image. Character/location sheets attached as image_references: EL CONDUCTOR, DINER DESIERTO.
ACTION: He chews once, swallows, and lets his eyes travel slowly from off-camera right back towards the lens without his head moving. He does not smile.
CAMERA: static shot with a slow push in, slight handheld drift.
...
SFX: room tone, cutlery on plates, a fan.
```
...
|---|---|---|
| `duration` | 4 | El plano dura 1.63 s, pero Seedance no genera clips de menos de 4 s. Se generan 4 s y en el montaje se recortan 1.63 s del centro, que es donde el movimiento ya esta asentado. |
...
> **Duracion:** **El plano dura 1.63 s, pero Seedance no genera clips de menos de 4 s.** Se generan 4 s y en el montaje se recortan 1.63 s del centro, que es donde el movimiento ya esta asentado. Los primeros frames de un clip generado casi siempre arrancan con un titubeo.
...
- [ ] ¿Hay al menos 1.63 s utiles seguidos, sin el titubeo del arranque?
...
**Anterior:** plano 02 · **Siguiente:** plano 04  
