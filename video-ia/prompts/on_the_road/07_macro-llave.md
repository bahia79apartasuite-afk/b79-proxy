# Plano 07 — macro llave

**Video** `on_the_road` · **In** 11.83 s · **Out** 12.87 s · **Duracion** 1.03 s (31 frames)  
**Tipo** macro · **Camara** macro fija · **Sujeto** PROP llave de contacto  
**Frame de referencia del original:** `analysis/on_the_road/frames/t_12.25.png`

> **Por que existe este plano:** El gesto de arranque. Un inserto de mano vale mas que un plano del motor.
...
Extreme close-up of a worn brass key on a plain split ring already seated in a chromed ignition barrel on a matte black dash panel, a dark-skinned hand on it. The yellow #F2BB34 body panel far out of focus behind. Hard warm side light from camera-left, shallow depth of field. The face is NOT in frame.
```

**Que lleva y en que orden:** bloque de estilo → bloque de INSERTOS COCHE → bloque de COCHE AMARILLO → encuadre y accion congelada. El orden importa: los modelos pesan mas el principio del prompt, y el estilo es lo que no puede fallar.

**Si usas Elements de Higgsfield:** en vez de pegar los bloques enteros, escribe `<<<id_de_insertos_coche>>>`, `<<<id_de_coche_amarillo>>>` dentro del prompt y deja solo el bloque de estilo y el encuadre. Los ids estan en `props/insertos_coche/element.txt`.
...
REFERENCES: start frame attached as start_image. Character/location sheets attached as image_references: INSERTOS COCHE, COCHE AMARILLO.
ACTION: The fingers rotate the key a quarter turn clockwise. The split ring swings and settles. The panel behind vibrates very slightly as the engine catches.
...
SFX: key detent, starter motor, a big V8 catching and settling into idle.
```
...
**Anterior:** plano 06 · **Siguiente:** plano 08  
